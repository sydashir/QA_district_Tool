...
async def enumerate_pages(client, config: BrandConfig):
    ...
    if config.wp_rest and config.wp_rest.enabled:
        ...
        # Only RETURN if WP-REST actually produced something. Returning an empty set here made the
        # static-list fallback below unreachable for exactly the brand it exists for: DBH's
        # WP-REST is gone with the rest of WordPress, so it yields 0 and there is still a page
        # list to fall through to.
        # Only fall PAST an empty WP-REST when there is actually a static list to fall through to.
        # Otherwise return it as before — every other brand relies on this path, and changing what
        # an empty result reports would alter their enumeration meta for no gain.
        if rest_urls or not getattr(config, "urls_file", None):
            return rest_urls, method, {"reason": reason, "post_types": config.wp_rest.post_types,
                                       "failed_sitemaps": failed_sitemaps}

    # THIRD AND LAST RESORT: a static URL list from the brand config.
    #
    # DBH was replatformed to headless WordPress behind Next.js on 2026-08-08. It serves NO
    # sitemap, NO robots.txt and NO WP-REST — every one of those paths returns the same 902 KB
    # app shell with a 404. The content is still fully auditable (server-prerendered, still
    # Elementor-shaped) and the URL structure survived the migration (30 of 30 sampled
    # pre-migration URLs still return 200), so a page list restores the brand.
    #
    # LIMITATION, and it is permanent, not a temporary workaround: a static list CANNOT DISCOVER
    # PAGES ADDED LATER. On this platform there is no index to read, so anything published after
    # the list was generated is invisible to the audit and will silently never be checked. The
    # list must be regenerated by hand whenever the brand adds pages. Say so in any report that
    # covers a brand enumerated this way — an audit that cannot see a page must never read as an
    # audit that found nothing wrong with it.
    if getattr(config, "urls_file", None):
        path = Path(config.urls_file)
        if not path.is_absolute():
            path = Path(__file__).resolve().parent.parent / path
        try:
            raw = path.read_text(encoding="utf-8").splitlines()
        except OSError as e:
            return [], "none", {"reason": f"urls_file unreadable: {e}", "urls_file": str(path)}
        listed = _apply_exclude(
            [ln.strip() for ln in raw if ln.strip() and not ln.lstrip().startswith("#")],
            config.crawl.exclude)
        if listed:
            return listed, "urls-file", {
                "reason": "no sitemap and no WP-REST (see crawl.enumerate_pages)",
                "urls_file": str(path), "count": len(listed),
                "cannot_discover_new_pages": True}
