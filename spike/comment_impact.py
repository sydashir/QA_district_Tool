"""Measure what the HTML-COMMENT bug did to EVERY check that reads visible_text.
...
from auditor.checks import blank, empty_slot, misspelling, phone, placeholder, scope, spelling
...
def old_visible_text(html: str, extra_css: str = "") -> str:
    """visible_text as it was BEFORE HTML comments were excluded — identical to _visible_text
    except that Comment/CData/Doctype nodes are read as page copy, which is what bs4's
    NavigableString subclassing made happen."""
    from bs4 import NavigableString
    soup = BeautifulSoup(html, "lxml")
    P._strip_hidden(soup, extra_css)
    ...
    for el in soup.descendants:
        if isinstance(el, NavigableString):
            ...
            if out and out[-1] != "\n" and out[-1][-1:].isalnum() and s[:1].isalnum():
                out.append(" ")
            out.append(s)
        elif el.name in P._BLOCK_SET:
            out.append("\n")
    t = P._INLINE_WS.sub(" ", "".join(out))
    return P._BLANK_LINES.sub("\n", t).strip()
...
async def measure(brand: str, n_pages: int) -> None:
    ...
    async with C.make_client(cfg.crawl) as client:
        ...
        for u in sample:
            ...
            old = dataclasses.replace(new, visible_text=old_visible_text(html, brand_css.css))
